...
# Formats soundfile often cannot open; decode via ffmpeg first.
_FFMPEG_EXTS = {".mp3", ".m4a", ".aac", ".mp4", ".ogg", ".wma", ".webm"}


def _load_wav_tensor(audio_path: str) -> Tuple[torch.Tensor, int]:
    """Load audio as (waveform, sample_rate). Uses ffmpeg when needed."""
    ...
    if shutil.which("ffmpeg") is None:
        raise RuntimeError(
            f"Cannot decode {ext} audio: ffmpeg not found on PATH. "
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"ffmpeg failed to decode {audio_path}: {err}")
...
def _iter_wav_segments_loaded(
    ...
    """Load the whole file then slice windows. More reliable on Windows."""
    wav, orig_sr = _load_wav_tensor(audio_path)
    wav = wav.mean(0)
    if orig_sr != sample_rate:
        wav = torchaudio.functional.resample(
            wav, orig_freq=orig_sr, new_freq=sample_rate
        )
    ...
    if n <= chunk_samples:
        if pad_last:
            yield F.pad(wav, (0, chunk_samples - n))
        else:
            yield wav
        return
    ...
    if pad_last and start < n:
        tail = wav[start:]
        yield F.pad(tail, (0, chunk_samples - int(tail.shape[0])))
...
def get_wav_segments(
    ...
    if os.name == "nt":
        yield from _iter_wav_segments_loaded(
