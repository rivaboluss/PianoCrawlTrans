...
def _get_silent_intervals(wav: torch.Tensor, top_db: float = 45.0):
    FRAME_LEN = 2048
    HOP_LEN = 512
    MIN_WINDOW_S = 5
...
def watchdog(main_pids: List, child_pids: List):
    while True:
        if not all(_pid_alive(pid) for pid in main_pids):
...
def batch_transcribe(
    ...
    if os.name != "posix" and compile_mode:
        print("torch.compile is not used on Windows; continuing without it")
        compile_mode = False
